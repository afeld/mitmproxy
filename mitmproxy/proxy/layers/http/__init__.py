    ...
    def handle_protocol_error(
        ...
        if isinstance(event, ResponseProtocolError):
            if self.client_state != self.state_errored:
                can_override_connect_failed = (
                    event.code is ErrorCode.CONNECT_FAILED
                    and self.server_state == self.state_wait_for_response_headers
                    and not had_response_before_hook
